...
func thread_download(f string, t_number int) int {
	...
	// get the filename
	var filename string = uri[(strings.LastIndex(uri, "/") + 1):len(uri)] //delete the slash
	log.Printf("thread %d: downloading %s\n", t_number, filename)

	dst, err := os.Create(filename)
	if err != nil {
		log.Panic("Error creating file: %s", filename)
		return 1
	}

	resp, err := http.Get(uri)
	if err != nil {
		log.Panic("Error GETting the file!")
		return 1
	}
	defer resp.Body.Close() //remember to close everything
	bytes, _ := io.Copy(dst, resp.Body)

	log.Printf("%s downloaded (%d bytes copied).", filename, bytes)
